...
const testimonials = [
  {
    id: 1,
    content: "Partnering with this solar farm was one of the best decisions for our land and our future. Not only did we reduce energy costs but we also became part of a cleaner greener initiative.",
    author: "James Tan",
  ...
  {
    id: 2,
    content: "The industrial scale efficiency provided by Ecoray has transformed our operational overhead. We've seen a 40% reduction in costs within the first year alone.",
    author: "Sarah Wick",
...
export default function AgroTestimonials() {
  const [active, setActive] = useState(0);

  const next = () => setActive((addr) => (addr + 1) % testimonials.length);
  const prev = () => setActive((addr) => (addr - 1 + testimonials.length) % testimonials.length);
    ...
    <section className="py-24 bg-agro-sky/10 border-y border-border/50 overflow-hidden">
      <div className="container mx-auto px-6">
        <div className="flex flex-col items-center text-center mb-16">
          ...
          >
            Hear From Those Who Power <br />
            <span className="text-muted-foreground font-medium">the Future With Us.</span>
          </motion.h2>
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="bg-white rounded-[3rem] p-8 md:p-16 shadow-2xl shadow-black/5 border border-white flex flex-col md:flex-row gap-12 items-center relative">
            ...
            <div className="flex flex-col text-left z-10">
              ...
              <div>
                <h4 className="text-xl font-bold text-agro-dark">{testimonials[active].author}</h4>
                <p className="text-sm text-primary font-bold uppercase tracking-widest">{testimonials[active].role}</p>
              </div>
